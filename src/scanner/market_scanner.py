...
try:
    ...
    from src.signals.score_engine  import compute_score, apply_ml_effect, apply_ai_effect
    ...
    try:
        ...
        from src.signals.regime_gating import (
            resolve_ml_threshold as _resolve_ml_threshold,
...
class MarketScanner:
    ...
    def _reject_signal(
        ...
        *,
        ...
        log_rejection(sig.signal_type, sig.asset, self.timeframe, reason_text)
        if save_rejection:
            save_signal(sig)
    ...
    def _classify_regime(self, sym: str, ha_df: object) -> Optional[Any]:
        ...
        try:
            ...
            if _regime_should_persist(snapshot, prev_snap):
                try:
                    _save_regime_snapshot(snapshot)
            ...
            return _build_regime_ctx(snapshot, previous_snapshot=prev_snap)
        except Exception as exc:
    ...
    def _scan_once(self) -> None:
        ...
        try:
            from src.ml.train import maybe_auto_retrain
            maybe_auto_retrain()
        except Exception:
            pass
        ...
        # Step 2b: Final Sprint prefilter layer
        ...
        for sc in top:
            ...
            if _ha is not None and "volume" in _ha.columns and len(_ha) >= 20:
                _avg_vol = float(_ha["volume"].tail(20).mean())
            pfr = run_prefilter(
                symbol=sc.symbol,
                atr_pct=sc.atr_pct,
        ...
        prefilter_passed = _select_top_prefilter(prefilter_results, top_n=self.top_candidates)
        ...
        if prefilter_results:
            ...
            top_survivors = ", ".join(
                f"{r.symbol}(atr={r.atr_pct:.2f}%,vol={r.volume_ratio:.2f}x,score={r.rank_score:.2f})"
                for r in prefilter_passed[:5]
            ) or "none"
            other_blocked = sum(
                count for reason, count in skip_counts.items()
                if reason not in {
                    "passed",
                    "blocked_by_low_volatility",
                    "blocked_memecoin_low_volatility",
                    "blocked_by_weak_volume",
                    "blocked_memecoin_weak_volume",
                    "blocked_below_rank_cutoff",
                }
            )
            log.info(
                "Prefilter summary [%s]: ranked=%d passed=%d low_vol=%d weak_vol=%d rank_cut=%d other=%d survivors=%s",
                self.timeframe,
                len(prefilter_results),
                len(prefilter_passed),
                skip_counts.get("blocked_by_low_volatility", 0) + skip_counts.get("blocked_memecoin_low_volatility", 0),
                skip_counts.get("blocked_by_weak_volume", 0) + skip_counts.get("blocked_memecoin_weak_volume", 0),
                skip_counts.get("blocked_below_rank_cutoff", 0),
                other_blocked,
                top_survivors,
            )
    ...
    def _evaluate_symbol(self, sym: str, ha_df: pd.DataFrame) -> None:
        ...
        for sig_key in ("buy", "sell"):
            ...
            # ML filter
            # Build enriched candle frame for ML feature extraction (canonical contract).
            try:
                ...
                ml_df = calculate_alligator(ha_df)
                ml_df = calculate_vortex(ml_df)
                ml_df = calculate_stochastic(ml_df)

                # ATR (HA) + volatility
                atr = calculate_atr(ml_df, period=14)
                ml_df = ml_df.copy()
                ml_df["atr_14"] = atr
                rets = ml_df["ha_close"].pct_change()
                ml_df["volatility_10"] = rets.rolling(10).std()
                ml_df["volatility_20"] = rets.rolling(20).std()

                # Ensure time exists and is datetime
                if "time" in ml_df.columns:
                    ml_df["time"] = pd.to_datetime(ml_df["time"], utc=True, errors="coerce")

                # Attach for ML model access (avoids changing model API everywhere)
                setattr(sig, "_ml_frame", ml_df)
            except Exception:
                setattr(sig, "_ml_frame", None)

            # Phase 5: compute score sub-components (uses ml_df for candle/volatility)
            _score_df = getattr(sig, "_ml_frame", None)
            ...
            if _REGIME_AVAILABLE and _REGIME_ADAPTER_AVAILABLE and regime_ctx is not None:
                _apply_regime_score_bias(sig, regime_ctx)
            ...
            if regime_ctx is not None:
                ...
                sig.macro_regime = _mr.value if hasattr(_mr, "value") else _mr
                sig.regime_label = _rl.value if hasattr(_rl, "value") else _rl
            ...
            # Phase 5: apply ML effect to score
            # Phase 11: use regime-adjusted ML threshold (fails open when regime unavailable)
            _ml_threshold = (
                _resolve_ml_threshold(ML_CONFIDENCE_THRESHOLD, regime_ctx)
                if _REGIME_AVAILABLE else ML_CONFIDENCE_THRESHOLD
            )
            apply_ml_effect(sig, ml_prob, _ml_threshold)
            if not passed_ml:
                self._reject_signal(sig, ha_df, f"ML filter rejected (prob={ml_prob:.0%})")
                continue
            ...
            if _REGIME_AVAILABLE and _REGIME_ADAPTER_AVAILABLE and regime_ctx is not None:
                ...
                _entry_ok, _entry_reason = _check_regime_entry_filter(
            ...
            _ml = sig.ml_confidence
            _ai = sig.ai_confidence
            if _ml is not None: _rc += f":ml{int(_ml * 100)}"
    ...
    def _open_position(self, sig: BuySignalResult | SellSignalResult, ha_df: pd.DataFrame) -> Optional[str]:
        ...
        if _REGIME_AVAILABLE and _regime_ctx is not None:
            _size_factor = _resolve_size_factor(_regime_ctx)
            if _size_factor != 1.0:
        ...
        if _REGIME_AVAILABLE and _REGIME_ADAPTER_AVAILABLE and _regime_ctx is not None:
            _adapted_gb, _adapted_be, _adapted_fade, _exit_reason = _adapt_exit_params(
        ...
        _ai   = sig.ai_confidence
        _conf = ""
        if _ml is not None or _ai is not None:
            parts = []
            if _ml is not None: parts.append(f"ml={_ml*100:.0f}%")
            if _ai is not None: parts.append(f"ai={_ai*100:.0f}%")
            _conf = " | " + " ".join(parts)
    ...
    def _update_open_positions(self, ha_data: dict) -> None:
        ...
        for tid, position_data in list(self._open.items()):
            # Always unpack as 4-tuple (rec, trail, tp_track, alligator_tp)
            if len(position_data) == 4:
                rec, trail, tp_track, alligator_tp = position_data
            elif len(position_data) == 3:
                rec, trail, tp_track = position_data
                alligator_tp = None
            else:
                raise ValueError(f"Unexpected tuple size in self._open: {len(position_data)}")
            ...
            if (
                ...
                try:
                    from src.indicators.utils import calculate_atr
                    _atr_val = calculate_atr(ha_df, period=14)
                    if _atr_val and _atr_val > 0:
                        ...
                        _new_stop   = trail.update(_atr_candidate)
                ...
                try:
                    ...
                    if len(ha_df) >= _needed:
                        ...
                        if _fa.fade_detected:
                            ...
                            try:
                                from src.indicators.utils import calculate_atr
                                _atr_val2 = calculate_atr(ha_df, period=14)
                            except Exception:
                                _atr_val2 = None
                            if _atr_val2 and _atr_val2 > 0:
                                ...
                                if abs(_new_stop2 - _old_stop2) > 1e-10:
                                    ...
                                    update_trade_lifecycle(
                                        ...
                                        trail_active_mode="candle_trail",
                                        exit_policy_name=rec.exit_policy_name,
